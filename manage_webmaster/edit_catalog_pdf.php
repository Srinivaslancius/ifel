...
if (!isset($_POST['submit']))  {
  ...
  if($_FILES["fileToUpload"]["name"]!='') {
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
          $sql = "UPDATE `catalog_pdf` SET pdf_name = '$fileToUpload', status='$status' WHERE id = '$id' ";
      ...
      $sql = "UPDATE `catalog_pdf` SET status='$status' WHERE id = '$id' ";
      if($conn->query($sql) === TRUE){
         echo "<script type='text/javascript'>window.location='catalog_pdf.php?msg=success'</script>";
      } else {
         echo "<script type='text/javascript'>window.location='catalog_pdf.php?msg=fail'</script>";
...
<div class="site-content">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <?php $getCatalogPdf = getDataFromTables('catalog_pdf',$status=NULL,'id',$id,$activeStatus=NULL,$activeTop=NULL);
              $getCatalogPdfData = $getCatalogPdf->fetch_assoc(); ?>
                  ...
                  <div class="form-group">
                    ...
                    <select id="form-control-3" name="status" class="custom-select" data-error="This field is required." required>
                      ...
                      <?php while($row = $getStatus->fetch_assoc()) {  ?>
                          <option <?php if($row['id'] == $getCatalogPdfData['status']) { echo "Selected"; } ?> value="<?php echo $row['id']; ?>"><?php echo $row['status']; ?></option>
                      <?php } ?>
                   </select>
